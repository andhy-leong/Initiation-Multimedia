...
async function fetchCountries() {
  try {
    ...
    countries.forEach(country => {
      ...
      scene.add(sprite);
      registerClickableFlag(sprite, lat, lon, name);
    });
...
map.on('click', (e) => {
  ...
  rotateEarthTo(lat, lng);
});

function rotateEarthTo(lat, lon) {
  const target = latLonToVector3(lat, lon, 3);

  const rotationY = THREE.MathUtils.degToRad(lon);
  const rotationX = THREE.MathUtils.degToRad(-lat);
...
async function addLeafletMarkers() {
  try {
    ...
    console.log('Marqueurs Leaflet chargés');
  } catch (err) {
...
const clickableFlags = [];

function registerClickableFlag(sprite, lat, lon, name) {
  sprite.userData = { lat, lon, name };
  clickableFlags.push(sprite);
}
...
renderer.domElement.addEventListener('click', (event) => {
  ...
  const intersects = raycaster.intersectObjects(clickableFlags, true);

  if (intersects.length > 0) {
    const obj = intersects[0].object;
    const { lat, lon, name } = obj.userData;
    console.log(`Clic sur : ${name} (${lat}, ${lon})`);
